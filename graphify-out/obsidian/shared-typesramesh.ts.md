---
source_file: "shared-types/ramesh.ts"
type: "code"
community: "Sales Analytics Core"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Sales_Analytics_Core
---

# shared-types/ramesh.ts

## Connections
- [[AnomalyEvidence]] - `imports` [EXTRACTED]
- [[DatasetType]] - `imports` [EXTRACTED]
- [[RAMESH_IDENTITY]] - `contains` [EXTRACTED]
- [[RAMESH_OFF_TOPIC_REPLY]] - `contains` [EXTRACTED]
- [[RameshAnswer]] - `contains` [EXTRACTED]
- [[RameshCalculationStep]] - `contains` [EXTRACTED]
- [[RameshDataUsed]] - `contains` [EXTRACTED]
- [[RameshInsightLine]] - `contains` [EXTRACTED]
- [[RameshIntent]] - `contains` [EXTRACTED]
- [[RameshMessage]] - `contains` [EXTRACTED]
- [[RameshQuery]] - `contains` [EXTRACTED]
- [[RameshWidget.tsx]] - `imports_from` [EXTRACTED]
- [[analysis.ts]] - `imports_from` [EXTRACTED]
- [[apiramesh.ts]] - `imports_from` [EXTRACTED]
- [[engine.ts]] - `imports_from` [EXTRACTED]
- [[intents.ts]] - `imports_from` [EXTRACTED]
- [[rameshroutes.ts]] - `imports_from` [EXTRACTED]
- [[shared-typesdatasets.ts]] - `imports_from` [EXTRACTED]
- [[shared-typesintelligence.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Sales_Analytics_Core