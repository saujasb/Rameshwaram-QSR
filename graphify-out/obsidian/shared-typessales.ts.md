---
source_file: "shared-types/sales.ts"
type: "code"
community: "Sales Import Pipeline, Parsers & UI"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Sales_Import_Pipeline_Parsers__UI
---

# shared-types/sales.ts

## Connections
- [[BaseRecord]] - `imports` [EXTRACTED]
- [[ImportValidationStatus]] - `contains` [EXTRACTED]
- [[SALES_CHANNEL_LABELS]] - `contains` [EXTRACTED]
- [[SalesAnalyticsPage.tsx]] - `imports_from` [EXTRACTED]
- [[SalesCategoryTotal]] - `contains` [EXTRACTED]
- [[SalesChannel]] - `contains` [EXTRACTED]
- [[SalesChannelTotal]] - `contains` [EXTRACTED]
- [[SalesDailyTotal]] - `contains` [EXTRACTED]
- [[SalesImportBatch]] - `contains` [EXTRACTED]
- [[SalesImportPage.tsx]] - `imports_from` [EXTRACTED]
- [[SalesImportRejection]] - `contains` [EXTRACTED]
- [[SalesImportValidation]] - `contains` [EXTRACTED]
- [[SalesItemTotal]] - `contains` [EXTRACTED]
- [[SalesLineItem]] - `contains` [EXTRACTED]
- [[SalesSummary]] - `contains` [EXTRACTED]
- [[SalesTargetSetting]] - `contains` [EXTRACTED]
- [[apisales.ts]] - `imports_from` [EXTRACTED]
- [[detectFormat.ts]] - `imports_from` [EXTRACTED]
- [[entities.ts]] - `imports_from` [EXTRACTED]
- [[parseTypes.ts]] - `imports_from` [EXTRACTED]
- [[parserspetpooja.ts]] - `imports_from` [EXTRACTED]
- [[salesimportPipeline.ts]] - `imports_from` [EXTRACTED]
- [[salesrepository.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Sales_Import_Pipeline_Parsers__UI