---
source_file: "shared-types/entities.ts"
type: "code"
community: "Generic CRUD UI Components"
location: "L3"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Generic_CRUD_UI_Components
---

# BaseRecord

## Connections
- [[dot-create()]] - `references` [EXTRACTED]
- [[dot-update()]] - `references` [EXTRACTED]
- [[AttendanceRecord]] - `inherits` [EXTRACTED]
- [[ComplaintRecord]] - `inherits` [EXTRACTED]
- [[CrudHooks]] - `references` [EXTRACTED]
- [[CrudModulePage.tsx]] - `imports` [EXTRACTED]
- [[ExpenseRecord]] - `inherits` [EXTRACTED]
- [[InventoryItem]] - `inherits` [EXTRACTED]
- [[InventoryMovement]] - `inherits` [EXTRACTED]
- [[InventoryPage.tsx]] - `imports` [EXTRACTED]
- [[MaintenanceIssue]] - `inherits` [EXTRACTED]
- [[ManualOrderEntry]] - `inherits` [EXTRACTED]
- [[ProviderOrder]] - `inherits` [EXTRACTED]
- [[Purchase]] - `inherits` [EXTRACTED]
- [[SalesImportBatch]] - `inherits` [EXTRACTED]
- [[SalesLineItem]] - `inherits` [EXTRACTED]
- [[StaffMember]] - `inherits` [EXTRACTED]
- [[Supplier]] - `inherits` [EXTRACTED]
- [[Task]] - `inherits` [EXTRACTED]
- [[TaskDetailModal.tsx]] - `imports` [EXTRACTED]
- [[TasksPage.tsx]] - `imports` [EXTRACTED]
- [[WastageEntry]] - `inherits` [EXTRACTED]
- [[createCrudRouter.ts]] - `imports` [EXTRACTED]
- [[createEntityHooks.ts]] - `imports` [EXTRACTED]
- [[entities.ts]] - `contains` [EXTRACTED]
- [[shared-typesproviderOrders.ts]] - `imports` [EXTRACTED]
- [[shared-typessales.ts]] - `imports` [EXTRACTED]
- [[sharedrepository.ts]] - `imports` [EXTRACTED]

## Source
**From** `shared-types/entities.ts` **(starting line 3):**
```typescript
export interface BaseRecord {
  id: string;
  createdAt: string;
  updatedAt: string;
}
```

#graphify/code #graphify/EXTRACTED #community/Generic_CRUD_UI_Components