...
# GoSelfServe order-status sync adapter config

## Connections
- [[rameshwaram-qsr-server (Render web service)]] - `references` [EXTRACTED]

## Source
> [!note] Rationale
> Outbound-adapter credentials for POST /order/status on GoSelfServe's order service. Leaving them unset skips the sync (recorded as goselfserveSyncStatus: 'not_configured' per order) until confirmed.
