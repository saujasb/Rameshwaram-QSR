---
source_file: "shared-types/intelligence.ts"
type: "code"
community: "Business Intelligence & Anomalies"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Business_Intelligence__Anomalies
---

# shared-types/intelligence.ts

## Connections
- [[ANOMALY_KIND_LABELS]] - `contains` [EXTRACTED]
- [[Anomaly]] - `contains` [EXTRACTED]
- [[AnomalyEvidence]] - `contains` [EXTRACTED]
- [[AnomalyKind]] - `contains` [EXTRACTED]
- [[AnomalySeverity]] - `contains` [EXTRACTED]
- [[DatasetType]] - `imports` [EXTRACTED]
- [[HourlyBucket]] - `contains` [EXTRACTED]
- [[Insight]] - `contains` [EXTRACTED]
- [[InsightCategory]] - `contains` [EXTRACTED]
- [[IntelligencePage.tsx]] - `imports_from` [EXTRACTED]
- [[Metric]] - `contains` [EXTRACTED]
- [[MetricBasis]] - `contains` [EXTRACTED]
- [[ProductPerformanceRow]] - `contains` [EXTRACTED]
- [[ReconciliationRow]] - `contains` [EXTRACTED]
- [[ReconciliationSummary]] - `contains` [EXTRACTED]
- [[TodaysIntelligence]] - `contains` [EXTRACTED]
- [[TodaysIntelligencePanel.tsx]] - `imports_from` [EXTRACTED]
- [[TopInsightsPanel.tsx]] - `imports_from` [EXTRACTED]
- [[analysis.ts]] - `imports_from` [EXTRACTED]
- [[anomalies.ts]] - `imports_from` [EXTRACTED]
- [[apidatasets.ts]] - `imports_from` [EXTRACTED]
- [[apiintelligence.ts]] - `imports_from` [EXTRACTED]
- [[datasetsrepository.ts]] - `imports_from` [EXTRACTED]
- [[engine.ts]] - `imports_from` [EXTRACTED]
- [[insights.ts]] - `imports_from` [EXTRACTED]
- [[intelligenceroutes.ts]] - `imports_from` [EXTRACTED]
- [[labels.ts]] - `re_exports` [EXTRACTED]
- [[reconciliation.ts]] - `imports_from` [EXTRACTED]
- [[shared-typesdatasets.ts]] - `imports_from` [EXTRACTED]
- [[shared-typesramesh.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Business_Intelligence__Anomalies