...
async function buildActionCenter(): Promise<ActionCenterItem[]> {
  ...
  for (const task of await taskRepository.list()) {
  ...
  for (const issue of await maintenanceRepository.list()) {
  ...
  for (const complaint of await complaintRepository.list()) {
  ...
  for (const item of await inventoryRepository.list()) {
  ...
  for (const purchase of await purchaseRepository.list()) {
  ...
  for (const entry of await wastageRepository.list()) {
...
actionCenterRouter.get("/", async (_req, res) => {
  try {
    res.json(await buildActionCenter());
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Failed to build action center." });
  }
});
