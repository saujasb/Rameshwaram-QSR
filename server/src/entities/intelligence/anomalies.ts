...
async function productionVariance(filter: DatasetFilter): Promise<Candidate[]> {
  ...
  for (const day of productionDays.slice(-MAX_PRODUCTION_DAYS)) {
    ...
    for (const row of await productPerformance({ ...filter, from: date, to: date })) {
...
async function wastageSurges(filter: DatasetFilter): Promise<Candidate[]> {
  const perf = await productPerformance(filter);
  const products = perf
...
async function productStalls(filter: DatasetFilter): Promise<Candidate[]> {
  ...
  for (const date of dates) {
    for (const row of await productPerformance({ ...filter, from: date, to: date })) {
      ...
      const priorSalesRaw = await dailyTotals({
        ...filter,
        datasetType: "sales",
        product: row.product,
        from: undefined,
      ...
      const priorSales = priorSalesRaw.filter((d) => d.quantity > 0);
