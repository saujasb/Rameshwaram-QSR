...
async function resolveBusinessDate(requested?: string): Promise<string> {
  ...
  return latest ?? getCurrentBusinessDate(await getBusinessDayStartHour());
}
...
intelligenceRouter.get("/today", async (req, res) => {
