...
async function answerCompareDatasets(c: Ctx): Promise<RameshAnswer> {
  ...
  if (prod && sale) {
    ...
    const [prodDatesRaw, saleDatesRaw] = await Promise.all([
      dailyTotals({ ...c.base, datasetType: "production" }),
      dailyTotals({ ...c.base, datasetType: "sales" }),
    ]);
    const prodDates = new Set(prodDatesRaw.map((d) => d.businessDate));
    const saleDates = new Set(saleDatesRaw.map((d) => d.businessDate));
